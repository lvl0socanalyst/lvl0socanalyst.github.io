...
A browser extension that prevents dodgy JavaScript from ruining your browsing experience. 

With Clickfix prompts on the rise, malicious browser extensions and drive by downloads tearing apart the real OS (the browser) I thought I'd give a safe browsing extension a crack.
...
It'll be open-source so anyone can clone and get into it. I think there is a bit of a gap between NoScript and a fully fledged Browser Based Detection and Response (BDR) solution. 
Neither are very attractive for a regular user. NoScript for the ultra paranoid and BDR for enterprise browsers trying to safeguard negligent employees. 

But what will the average user use? BrowserBuddy! (ofc)
...
All the following features are either in dev or complete. 

### Blocking JavaScript Clipboard APIs (DONE)

First successful addition to this is the blocking of JS Clipboard APIs. 

There are two JS APIs that trigger JS programmatic copying.
...
- Users unable to interact with copy buttons on websites.
- However, can still highlight --> right click --> copy
...
I recently wrote an article on Clickfix being used for Push Notification subscribing by an interesting Moroccan dude [Push Notification ClickFix](https://lvl0socanalyst.github.io/malware/analysis/2025/09/20/my-25th-post.html)

In summary I found the prompt was subscribing browsers to push notifications. These subscriptions we're then being sold off for crypto on the threat actors website. 

Anywho, got me thinking I should add this as a feature. Because honestly, when is this used legitimately nowdays?
...
This is something that enterprise BDR solutions implement. I'm almost certain this is signature based. However, will need to deep dive to figure this out
