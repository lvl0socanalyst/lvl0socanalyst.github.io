...
4. If successful, it will return the signing key. In my case it is 'secret1'
5. Open Burp Decoder up --> Convert signing key to B64
6. Go to JWT Editor --> New Symmetric Key --> Generate Key --> Modify 'k' value to your B64 encoded signing key
![Everyone](/images/sym_key.PNG)
7. Open up Burp Repeater with a POST request with a JWT token.
8. Click on 'JSON Web Token' tab
9. Modify 'sub' value to intended value. In this case it was 'administrator' as the JWT token was used in a POST request to login a user.
10. Sign your new JWT with the signing key we created in the previous steps. 
![Everyone](/images/signing.PNG)
11. Hit Send
12. Right Click --> Open response in browser --> Copy and Paste
13. Note - You have to repeat the signing process for every request you send.
