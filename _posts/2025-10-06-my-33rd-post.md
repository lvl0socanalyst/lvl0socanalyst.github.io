...
Getting closer. This looks like an XOR function. Passes in our XOR key which we know is the value of ``$golKv`` and iterates through ``$hexafoil`` to decrypt the goodies.
...
Lets run my strings IOC script over it and see what we can find. No hits, that hurts the ego. DiE suggested heavy obfuscation so not overly surprised with this result.
...
Opening up the .exe in DNSpy is a complete headache, obfuscation galore.

So, its dynamic time! I'd love to know exactly what this is. 

## Dynamic Analysis

## Other interesting code I'm yet to look at

Elf file
How "AMSI_RESULT_NOT_DETECTED" was calculated