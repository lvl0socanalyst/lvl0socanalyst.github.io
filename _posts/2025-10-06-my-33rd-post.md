...
Few spoilers include a three staged dropper/loader, AMSI bypass code, XOR everywhere and Malware Dev OPSec errors.
...
I tried to rebuild the .elf file in a Linux VM but cannot get it right, probably a character or a space off the right bits and bytes. I'll leave it for now. Perhaps later on they're may be opportunity to dump it.

![alt text](/images/vbs.PNG)

Looks like some hex strings.I'm seeing this throughout the code, so I'll go through and convert every hex string I spot.
...
Essentially the function uses WMI to spawn a hidden process. What I want is what is passed to the function ``nrhfhprneb`` because that is likely a command/script.
...
Then with the B64 left over, I'll copy/paste to a new input tab and decode that.
...
Looks like inline encoding. What we have is a big old string. The decoding function at the end of the string, looks complicated. So time to whip out CyberChef again, time to unbake a payload. 
...
I'll repull a fresh sample of the dropper, apply the above as domains have likely been rolled. (Skip to "Loader" section to pick up from here).
...
Scrap that this is easy (amazing what sleep can do).
...
Well it looks like we've found the next stage of whatever on earth we are dealing with here. Appreciate the comments explaining the code, lvl 0 soc analyst approved. No obfuscation either, how good.
