...
Sister called. Downloaded a "pdf", that when opened had some "weird stuff" pop up. 
...
Quotation_pdf.htm
```
HTM files are technically .html files however are far more legacy, they are opened by browsers and display hypertext markup to a user.
...
Likely the account of Oriana Rybinska has been compromised rather than the entire domain.
...
Ohhhh and before I forget here is the landing page.
...
Well few things we can pull from this. "Mark_hill05" sounds a lot like a legit account. The reason I suspect it's legit is because of the 05 at the end, insinuating the user's age. But could be wrong.

But before I forget let's circle back on BigDrakes quote of the year. I'm almost in tears. So inspirational.
...
This mimicks what we saw in the .JS earlier. Therefore, the logs we are seeing are definitely from there.
...
When I try enumerating those earlier messages it fails, likely because the message was deleted.
...
IP information is useful for an attacker as they can connect to a proxy/VPN in a given area to try and bypass conditional access policies (CAP) for the account. A lot of modern web applications will block a user logining in from China if they usually login from Australia (CAP block). The method of noting down victim IPs hopes to bypass this control.

### BigDrake00 Logs

Yikes, after sending a test message to BigDrake00 as the bot I received the messageID 49172. Therefore, indicating that I have sent the 49172nd message to BigDrake00 as the bot. Another crap load of logs.

These logs are pullable from messageID 0 upwards, indicating BigDrake00 hasn't purged any older logs unlike mark_hill05.

### Next Steps
1. Dig into the Nigerian IPs and fuzz data that is likely the attackers
2. Look into /sendDocument API and others that provide bot functionality.
3. Use /editMessageText API to edit the logs I have sent to the user.
