...
From what we've observed above we can see it's compiled using .NET. Therefore, DNspy is the only viable option for a lvl 0 analyst.

![Stage 19](/images/type_refs_dnspy.PNG)

Off the bat we can see some pretty interesting type references being called. Bitmap and AES. Steganography? 

![Stage 20](/images/AES_decrypted_payload.PNG)

Pivoting to our block of code we can see an AES decryption function. Potentially a payload/command?
- Passes in string, key and IV
...
Alright. So this is cool. We should have a payload/command we can see if we get our hands on this result variable value.

Let's try and do up a script to give us this value.

========SCRIPT=========

![Stage 20](/images/base64_dnspy_encrypted.PNG)

This one is way easier to comprehend. Looks like an AES encrypted B64 string. With all the goodies in clear text for us.
